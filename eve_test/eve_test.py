import pyautogui as pg
from pynput import keyboard
from time import sleep
import os
import json
from time import time

isRunning = True

def main():

    initializePyAutoGUI()
    countdownTimer()


    isStationFound = False

    while isRunning:   

     # проверяем что корабль в доке
     if get_location("station"):
            # запускаем скрипт выхода из дока
            playActions("actions_station_out.json")
            isStationFound = False
            # ждем
            sleep(10)
     else:
         # если корабль в космосе 
         if get_location("space"):           
           # и станция не найдена
           if not isStationFound:
             # пробуем найти станцию
             for i in (0,5):         
               # пытаемся выделить обьект "Станция"
               playActions("actions_station_in.json")
               # проверяем что обьект "Станция" выделен
               isStationFound=get_location("object")
               if isStationFound:
                  break;         
           # запускаем скрипт входа в док
           if isStationFound:
            playActions("actions_station_in_1.json")
            # ждем
            sleep(45)

def initializePyAutoGUI():
    # Initialized PyAutoGUI
    # https://pyautogui.readthedocs.io/en/latest/introduction.html
    # When fail-safe mode is True, moving the mouse to the upper-left corner will abort your program.
    pg.FAILSAFE = True


def countdownTimer():
    # Countdown timer
    print("Starting", end="", flush=True)
    for i in range(0, 5):
        print(".", end="", flush=True)
        sleep(1)
    print("Go")

def get_location(station):
    # set the list of images we are trying to match
    if station == 'station':
        images_to_check = [                             
            'eve_station_2.png',                        
        ]        
    elif station == 'object':
        images_to_check = [                             
            'object.png',                        
        ]
    elif station == 'space':
        images_to_check = [                             
            'eve_space.png',                        
        ]
    
    # loop over images until one is found, then return the index of the found
    # image
    for index, image_filename in enumerate(images_to_check):
        script_dir = os.path.dirname(__file__)
        needle_path = os.path.join(
            script_dir, 
            'Images', 
            image_filename
        )
        image_pos = pg.locateOnScreen(needle_path)   
        s = station
        if image_pos:
            print('Location found - {}'.format(s))
            return True

    return False


def playActions(filename):
    # Read the file
    script_dir = os.path.dirname(__file__)
    filepath = os.path.join(
        script_dir, 
        'recordings', 
        filename
    )
    with open(filepath, 'r') as jsonfile:
        # parse the json
        data = json.load(jsonfile)
        
        # loop over each action
        # Because we are not waiting any time before executing the first action, any delay before the initial
        # action is recorded will not be reflected in the playback.
        for index, action in enumerate(data):
            action_start_time = time()

            # look for escape input to exit
            if action['button'] == 'Key.esc':
                break

            # perform the action
            if action['type'] == 'keyDown':
                key = convertKey(action['button'])
                pg.keyDown(key)
                print("keyDown on {}".format(key))
            elif action['type'] == 'keyUp':
                key = convertKey(action['button'])
                pg.keyUp(key)
                print("keyUp on {}".format(key))
            elif action['type'] == 'click':
                pg.click(action['pos'][0], action['pos'][1], duration=0.25)
                print("click on {}".format(action['pos']))

            # then sleep until next action should occur
            try:
                next_action = data[index + 1]
            except IndexError:
                # this was the last action in the list
                break
            elapsed_time = next_action['time'] - action['time']

            # if elapsed_time is negative, that means our actions are not ordered correctly. throw an error
            if elapsed_time < 0:
                raise Exception('Unexpected action ordering.')

            # adjust elapsed_time to account for our code taking time to run
            elapsed_time -= (time() - action_start_time)
            if elapsed_time < 0:
                elapsed_time = 0
            print('sleeping for {}'.format(elapsed_time))
            sleep(elapsed_time)


            # convert pynput button keys into pyautogui keys
# https://pynput.readthedocs.io/en/latest/_modules/pynput/keyboard/_base.html#Key
# https://pyautogui.readthedocs.io/en/latest/keyboard.html
def convertKey(button):
    PYNPUT_SPECIAL_CASE_MAP = {
        'alt_l': 'altleft',
        'alt_r': 'altright',
        'alt_gr': 'altright',
        'caps_lock': 'capslock',
        'ctrl_l': 'ctrlleft',
        'ctrl_r': 'ctrlright',
        'page_down': 'pagedown',
        'page_up': 'pageup',
        'shift_l': 'shiftleft',
        'shift_r': 'shiftright',
        'num_lock': 'numlock',
        'print_screen': 'printscreen',
        'scroll_lock': 'scrolllock',
    }

    # example: 'Key.F9' should return 'F9', 'w' should return as 'w'
    cleaned_key = button.replace('Key.', '')

    if cleaned_key in PYNPUT_SPECIAL_CASE_MAP:
        return PYNPUT_SPECIAL_CASE_MAP[cleaned_key]

    return cleaned_key

def on_press(key):
    # we only want to record the first keypress event until that key has been 
    # released
    isisRunning = False


def runListeners():    

    # Collect keyboard inputs until released
    # https://pynput.readthedocs.io/en/latest/keyboard.html#monitoring-the-keyboard
    with keyboard.Listener(
            on_press=on_press) as listener:
        listener.join()

if __name__=="__main__":
    main()